...
def solve_Biharmonic3D_Dirichlet(R):
    ...
    if R['eqs_type'] == 'Biharmonic3D':
        f, u_true, u_bottom, u_top, u_left, u_right, u_front, u_behind = Biharmonic_eqs.get_biharmonic_Dirichlet_3D(
    ...
    with tf.device('/gpu:%s' % (R['gpuNo'])):
        with tf.variable_scope('vscope', reuse=tf.AUTO_REUSE):
            ...
            if R['variational_loss'] == 1:
                ...
                loss_it_variational = 0.5 * laplace_norm - tf.multiply(f(X_it, Y_it, Z_it), U_NN)
            ...
            # ubottom = UBottom_NN(XYZ_bottom_bd[:, 0], XYZ_bottom_bd[:, 1], XYZ_bottom_bd[:, 2])
            # utop = UTop_NN(XYZ_top_bd[:, 0], XYZ_top_bd[:, 1], XYZ_top_bd[:, 2])
            # uleft = ULeft_NN(XYZ_left_bd[:, 0], XYZ_left_bd[:, 1], XYZ_left_bd[:, 2])
            # uright = URight_NN(XYZ_right_bd[:, 0], XYZ_right_bd[:, 1], XYZ_right_bd[:, 2])
            # ufront = UFront_NN(XYZ_front_bd[:, 0], XYZ_front_bd[:, 1], XYZ_front_bd[:, 2])
            # ubehind = UBehind_NN(XYZ_behind_bd[:, 0], XYZ_behind_bd[:, 1], XYZ_behind_bd[:, 2])

            # loss_bd_square = tf.square(UBottom_NN - tf.reshape(ubottom, shape=[-1, 1])) + \
            #                  tf.square(UTop_NN - tf.reshape(utop, shape=[-1, 1])) + \
            #                  tf.square(ULeft_NN - tf.reshape(uleft, shape=[-1, 1])) + \
            #                  tf.square(URight_NN - tf.reshape(uright, shape=[-1, 1])) + \
            #                  tf.square(UFront_NN - tf.reshape(ufront, shape=[-1, 1])) + \
            #                  tf.square(UBehind_NN - tf.reshape(ubehind, shape=[-1, 1]))
            loss_bd_square = tf.square(UBottom_NN) + tf.square(UTop_NN) + tf.square(ULeft_NN) + \
                             tf.square(URight_NN) + tf.square(UFront_NN) + tf.square(UBehind_NN)
            ...
            loss = loss_it + boundary_penalty * loss_bd + boundary_penalty * loss_bd2derivative  # 要优化的loss function
    ...
    t0 = time.time()
    loss_it_all, loss_bd_all, loss_bdd_all, loss_all, train_mse_all, train_rel_all = [], [], [], [], [], []  # 空列表, 使用 append() 添加元素
    ...
    # size2test = 500
    test_xyz_bach = DNN_data.rand_it(test_bach_size, 3, region_lb, region_rt)
    ...
    with tf.Session(config=config) as sess:
        ...
        for i_epoch in range(R['max_epoch'] + 1):
            ...
            _, loss_it_tmp, loss_bd_tmp, loss_bd2_tmp, loss_tmp, train_mse_tmp, train_res_tmp, pwb = sess.run(
                [train_loss, loss_it, loss_bd, loss_bd2derivative, loss, train_mse, train_rel, PWB],
                feed_dict={XYZ_it: xyz_it_batch, XYZ_bottom_bd: xyz_bottom_batch, XYZ_top_bd: xyz_top_batch, XYZ_left_bd: xyz_left_batch,
                           XYZ_right_bd: xyz_right_batch, XYZ_front_bd: xyz_front_batch, XYZ_behind_bd: xyz_behind_batch,
                           in_learning_rate: tmp_lr, train_opt: train_option, boundary_penalty: temp_penalty_bd})
        ...
        saveData.save_trainLoss2mat_1actFunc_Dirichlet(loss_it_all, loss_bd_all, loss_bdd_all, loss_all,
...
if __name__ == "__main__":
    ...
    solve_Biharmonic3D_Dirichlet(R)
