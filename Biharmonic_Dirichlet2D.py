...
def solve_Biharmonic2D_Dirichlet(R):
    ...
    if R['eqs_type'] == 'general_Biharmonic':
        mu = 1.0
        # laplace laplace u = f
        f, u_true, u_left, u_right, u_bottom, u_top = Biharmonic_eqs.get_biharmonic_Dirichlet_2D(
    ...
    with tf.device('/gpu:%s' % (R['gpuNo'])):
        with tf.variable_scope('vscope', reuse=tf.AUTO_REUSE):
            ...
            if 'PDE_DNN' == str.upper(R['model']):
                ...
                U_NN = DNN_base.PDE_DNN_BN(XY_it, Weights, Biases, hidden_layers, activate_name=act_func,
                                           is_training=train_opt)
                ULeft_NN = DNN_base.PDE_DNN_BN(XY_left_bd, Weights, Biases, hidden_layers, activate_name=act_func,
                                               is_training=train_opt)
                URight_NN = DNN_base.PDE_DNN_BN(XY_right_bd, Weights, Biases, hidden_layers, activate_name=act_func,
                                                is_training=train_opt)
                UBottom_NN = DNN_base.PDE_DNN_BN(XY_bottom_bd, Weights, Biases, hidden_layers, activate_name=act_func,
                                                 is_training=train_opt)
                UTop_NN = DNN_base.PDE_DNN_BN(XY_top_bd, Weights, Biases, hidden_layers, activate_name=act_func,
                                              is_training=train_opt)
...
if __name__ == "__main__":
    ...
    # R['hidden_layers'] = (8, 8, 6, 4, 4, 2)    # it is used to debug our work
    ...
    solve_Biharmonic2D_Dirichlet(R)
