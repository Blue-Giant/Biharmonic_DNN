...
def solve_Biharmonic2D(R):
    ...
    with tf.device('/gpu:%s' % (R['gpuNo'])):
        with tf.variable_scope('vscope', reuse=tf.AUTO_REUSE):
            ...
            # U_left = u_left(XY_left_bd[:, 0], XY_left_bd[:, 1])
            # U_right = u_right(XY_right_bd[:, 0], XY_right_bd[:, 1])
            # U_bottom = u_bottom(XY_bottom_bd[:, 0], XY_bottom_bd[:, 1])
            # U_top = u_top(XY_top_bd[:, 0], XY_top_bd[:, 1])
            #
            # loss_bd_square = tf.square(ULeft_NN - tf.reshape(U_left, shape=[-1, 1])) + \
            #                  tf.square(URight_NN - tf.reshape(U_right, shape=[-1, 1])) + \
            #                  tf.square(UBottom_NN - tf.reshape(U_bottom, shape=[-1, 1])) + \
            #                  tf.square(UTop_NN - tf.reshape(U_top, shape=[-1, 1]))
            loss_bd_square = tf.square(ULeft_NN) + tf.square(URight_NN) + tf.square(UBottom_NN) + tf.square(UTop_NN)
    ...
    t0 = time.time()
    loss_it_all, loss_bd_all, loss_all, train_mse_all, train_res_all = [], [], [], [], []
    testing_mse_error, testing_res_error = [], []
    testing_epoch = []
    ...
    with tf.Session(config=config) as sess:
        ...
        for i_epoch in range(R['max_epoch'] + 1):
            ...
            _, loss_it_tmp, loss_bd_tmp, loss_bd2_tmp, loss_tmp, train_mse_tmp, train_res_tmp, pwb = sess.run(
            ...
            loss_all.append(loss_bd2_tmp)
            train_mse_all.append(train_mse_tmp)
            train_res_all.append(train_res_tmp)
            ...
            if i_epoch % 1000 == 0:
                ...
                train_option = False
                u_true2test, u_predict2test = sess.run(
                    [U_true, U_NN], feed_dict={XY_it: test_xy_bach, train_opt: train_option})
                point_square_error = np.square(u_true2test - u_predict2test)
                mse2test = np.mean(point_square_error)
                testing_mse_error.append(mse2test)
                res2test = mse2test/np.mean(np.square(u_true2test))
                testing_res_error.append(res2test)
                print('mean square error of predict and real for testing: %10f' % mse2test)
                print('residual error of predict and real for testing: %10f\n' % res2test)
            ...
            if (i_epoch != 0 or i_epoch != 100000) and i_epoch % 10000 == 0 and R['Navier_boundary'] == 1:
                plotData.plotTrain_MSE_REL_1act_func(testing_mse_error, testing_res_error, actName='s2ReLU',
                                                     seedNo=R['seed'], outPath=R['FolderName'], yaxis_scale=True)

                if R['hot_power'] == 0:
                    plotData.plot_scatter_solutions2test(u_true2test, u_predict2test, test_xy_bach, actName1='Utrue',
                                                         actName2='s2ReLU', seedNo=R['seed'], outPath=R['FolderName'])
                elif R['hot_power'] == 1:
                    # 绘解得热力图
                    plotData.plot_Hot_solution2test(u_true2test, size_vec2mat=size2test, actName='Utrue',
                                                    seedNo=R['seed'], outPath=R['FolderName'])
                    # 绘制预测解得热力图
                    plotData.plot_Hot_solution2test(u_predict2test, size_vec2mat=size2test, actName='s2ReLU',
                                                    seedNo=R['seed'], outPath=R['FolderName'])
        ...
        saveData.save_trainLoss2mat_1actFunc(loss_it_all, loss_bd_all, loss_all, actName='s2ReLU',
        ...
        saveData.save_train_MSE_REL2mat(train_mse_all, train_res_all, actName='s2ReLU', outPath=R['FolderName'])
        plotData.plotTrain_MSE_REL_1act_func(train_mse_all, train_res_all, actName='s2ReLU', seedNo=R['seed'],
                                             outPath=R['FolderName'], yaxis_scale=True)

        # ------------------------------ save testing result to mat file, then plot them -------------------------------
        saveData.save_testData_or_Results2mat(u_true2test, dataName='Utrue', outPath=R['FolderName'])
        saveData.save_testData_or_Results2mat(u_predict2test, dataName='Us2ReLU', outPath=R['FolderName'])
        if R['hot_power'] == 0:
            plotData.plot_scatter_solutions2test(u_true2test, u_predict2test, test_xy_bach, actName1='Utrue',
                                                 actName2='s2ReLU', seedNo=R['seed'], outPath=R['FolderName'])
        elif R['hot_power'] == 1:
        ...
        saveData.save_testMSE_REL2mat(testing_mse_error, testing_res_error, actName='s2ReLU', outPath=R['FolderName'])
        plotData.plotTest_MSE_REL(testing_mse_error, testing_res_error, testing_epoch, actName='s2ReLU',
                                  seedNo=R['seed'], outPath=R['FolderName'], yaxis_scale=True)

        # 绘制误差的能量图
        saveData.save_test_point_wise_err2mat(point_square_error, actName='s2ReLU', outPath=R['FolderName'])
        plotData.plot_Hot_point_wise_err(point_square_error, size_vec2mat=size2test, actName='s2ReLU',
                                         seedNo=R['seed'], outPath=R['FolderName'])
